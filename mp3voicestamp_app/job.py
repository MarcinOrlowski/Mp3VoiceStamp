...
class Job(object):
    ...
    def get_out_file_name(self, music_track, segment_label=' '):
    ...
    def __blend_wavs(self, speak_wav, title_wav, result_file_name):
    ...
    def __create_ticks_wav(self, speak_segments, speech_wav_file_name):
        ...
        for idx, _ in enumerate(speak_segments):
            os.unlink(os.path.join(self.__tmp_dir, '{}.wav'.format(idx)))
    ...
    def voice_stamp(self, mp3_file_name):
        ...
        try:
            ...
            if self.__config.verbose:
                ...
                Log.v('Duration: {} secs'.format(music_track.duration))
                Log.level_pop()
            ...
            Log.d('Segments: {} ({} minutes each)'.format(number_of_track_segments, track_segment_duration))
            ...
            if not self.__config.dry_run_mode:
                ...
                music_track.to_wav_segments(music_wav_full_path, self.__config.split_segment_duration)
            ...
            for segment_number in range(0, number_of_track_segments):
                ...
                segment_label = ' (segment {segment_number_zeros}) '.format(
                    segment_number=segment_number_for_users,
                    segment_number_zeros='%02d' % segment_number_for_users
                )
                segment_wav_file_name = self.get_out_file_name(music_track, segment_label)
                ...
                extras = {
                    ...
                    'segment_name': formatted_segment_name,
                }

                formatted_segment_name = Util.process_placeholders(track_segment_name_format, extras)
                ...
                if not self.__config.dry_run_mode:
                    ...
                    name, _ = Util.split_file_name(os.path.basename(music_track.file_name))
                    music_segment_wav_full_path = os.path.join(self.__tmp_dir, name + '.%d.wav' % segment_number)
                    ...
                    output_file_msg = 'Output file "{}"'.format(segment_wav_file_name)
