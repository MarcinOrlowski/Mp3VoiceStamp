...
class App(object):
    ...
    def process_file(self, job):
        ...
        if os.path.exists(job.file_out):
            if not job.force:
                Util.abort('Target "{}" already exists'.format(job.file_out))
            if not os.path.isfile(job.file_out):
                Util.abort('Target "{}" is not a file. Cannot overwrite'.format(job.file_out))
        ...
        Util.debug('Temporary folder: {}'.format(tmp_dir))
        ...
        for idx, segment_text in enumerate(segments):
            segment_file_name = os.path.join(tmp_dir, '{}.wav'.format(idx))
        ...
        wav = wave.open(os.path.join(tmp_dir, '0.wav'), 'rb')
        speech_meged_wav_full = os.path.join(tmp_dir, 'speech.wav')
        ...
        for idx, i in enumerate(segments):
            concat_cmd.extend(['-i', os.path.join(tmp_dir, '{}.wav'.format(idx))])
        ...
        # Also there are some odd issues with "ffmpeg" failing during merge when source was in mp3 - blind guess for now
        # it was due to some structure mismatch between MP3 file (i.e. having cover image), and speech segments being
        ...
        source_wav_full_path = os.path.join(tmp_dir, source_wav_file_name)
        ...
        merge_cmd = ['ffmpeg', '-y',
                     '-i', os.path.join(tmp_dir, source_wav_file_name),
                     '-i', os.path.join(tmp_dir, 'speech.wav'),
        ...
        # cleanup
        if not job.args.no_cleanup:
            if job.args.no_cleanup and tmp_dir is not None:
                shutil.rmtree(tmp_dir)
        else:
            if job.args.debug:
                Util.debug('Leaving "{}" folder intact'.format(tmp_dir))
    ...
    def main(self):
        ...
        for file_in in self.args.files_in:
            ...
            self.process_file(Job(self.args, file_in))
